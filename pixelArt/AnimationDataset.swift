...
protocol AnimationDatasetDelegate: class {
    var delegateID: String { get}
    func datasetUpdated()
}

// The actual data configuration
final class AnimationDataset {
    ...
    private final class WeakDatasetDelegate {
        weak var delegate: PixelDatasetDelegate?
        
        init(delegate: PixelDatasetDelegate) {
    ...
    private static func loadData() -> [Entry] {
        ...
        guard
            let fileURL: URL = try? FileManager.default.url(for: .documentDirectory, in: .userDomainMask, appropriateFor: nil, create: true).appendingPathComponent("AnimationDataset.json", isDirectory: false),
    ...
    // Get delegate
    static func registerDelegate(_ delegate: PixelDatasetDelegate) {
