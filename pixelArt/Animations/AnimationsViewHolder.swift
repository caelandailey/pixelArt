...
class AnimationsViewHolder: UIView, UIScrollViewDelegate {
    ...
    let animationControl: AnimationControl = {
        let animationControl = AnimationControl()
        animationControl.translatesAutoresizingMaskIntoConstraints = false
    ...
    init(frame: CGRect, sizeFactor: CGFloat) {
        ...
        animationControl.frame = CGRect(x: frame.width/3, y: 100, width: frame.width/3, height: frame.height/20)
